...
Edge detection is problematic because is should work on different light conditions, as well it should find different types of lines (white, yellow, etc), and preferebly it should not find the things which are not line (for example, shadows). My approach (combining different algorithms and channels) works pretty OK, and I think it could be developed even more. Maybe some color normalization could also help.
...
That's why I'm making before the polynominal fit some filtering however parameters a chosen manually; 4-STD from the mean is chosed manually, so there is no guarantee that it would work in every situation.
...
My solution to this problem is to filter out the 'huge jumps' between the point; but choosing the parameter to define a 'huge jump' is somewhat accidental.
